...
function SearchBar({ departments }) {
  ...
  return (
    <>
      ...
      {isLoading && (
        <>
          <svg role="status" className="h-8 w-8 animate-spin mr-2 text-gray-100 dark:text-gray-100 dark:fill-gray-900" viewBox="0 0 100 101" fill="none" xmlns="http://www.w3.org/2000/svg">
      ...
      {searched && !isLoading && (
        <>
          <h1 className='font-bold text-lg col-span-3'>
            { (searchItems?.results) ? `Number of Results: ${ [ searchItems?.results ]}` : '' }
          </h1>
